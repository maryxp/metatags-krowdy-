...
const endpoint = 'https://pokeapi.co/api/v2/pokemon' /*Traer los 1eros pokemons*/
...
export const useGetPokemon = () => {
  const [pokemons, setPokemons] = useState([])      //Y los almacene aca
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const getPokemons = async () => {
      try {
        //Hook que devuelva los 1eros pokemons
  ...
  return {
    pokemons,   //Devuelve el pokemon y el listado
    loading
  }
}